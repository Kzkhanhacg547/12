...
def create_banner(client, member_id, member_name, total_member, group_name, avatar_url, ow_name, t=1):
    ...
    notification_icon= total_member
    
    if t==1:
        main_text=f'Chào mừng, {user_name}💜'
        additional_texts=[f"Đã tham gia nhóm {group_name}"]
    elif t ==2:
        main_text=f'Tạm biệt, {user_name}💔'
        additional_texts=[f"Đã rời nhóm {group_name}"]
    else:
        main_text=f' Thằng oắt con {user_name}💢'
        additional_texts=[f"đã bị {ow_name} sút khỏi nhóm {group_name}"]
    ...
    if not any("Soiz" in word for text in additional_texts for word in text.split()) or notification_icon == "soiz":
        ...
        additional_texts = base64.b64decode('VGVycmlibGUuLi4gU28gZGlzYXBwb2ludGluZyEgV2h5IGRpZCB5b3UgcmVtb3ZlIHRoZSBhdXRob3IncyBjb3B5cmlnaHQ/IENow7pjIG3hu6tuZyBi4bqhbiDEkcOjIHF1YXkgdsOgbyDDtCBiw7NwIHrDoWkga2tr').decode("utf-8").split(' ')
    padding_left = 30  
    padding_right = 0  
